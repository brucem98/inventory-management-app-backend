...
const pgUser = "xifkwbke "

/**
 * This is our main entry point of our Express server.
 * All the routes in our API are going to be here.
 *
 * This code sets up an Express server with routes for handling
 * API requests and includes functionality to seed initial data
 * into the database using TypeORM.
 **/
...
const App = () => {
  ...
  app.use(cors())
  app.use(postgraphile("postgres://xifkwbke:83Uq29ghREmVLdnDWkKYMluthOe_rqrt@flora.db.elephantsql.com/xifkwbke", "public", {
    watchPg: true,
    graphiql: true,
  ...
  app.post("/api/v1/test/data", async (req, res, next) => {
    ...
    // Saves the created data to the database
